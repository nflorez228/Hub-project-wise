...
exports.getData = function(filename, start, end, dpcount) {
  ...
  var filesize = exports.fileSize(filename);
  var interval = (end - start) / dpcount;
  ...
  {
    var pos = exports.binarySearch(fd,start+(i*interval),filesize);
    //console.log(i +' pos: ' + pos);
    var last_time = timetmp;
    fs.readSync(fd, buff, 0, 9, pos);
    timetmp = buff.readUInt32BE(1);
    var value = buff.readInt32BE(5);
...
exports.binarySearch = function(fileDescriptor,timestamp, filesize) {
  ...
  for (var i=0; i<30; i++)
  {
    ...
    var mid = start + Math.round((end-start)/16)*9;
...
exports.binarySearchExact = function(fileDescriptor, timestamp, filesize) {
  ...
  var start = 0; var end = filesize-9;
  var buff = new Buffer(4);
  var tmp = 0;
  for (var i=0; i<30; i++)
  {
    var mid = start + Math.round((end-start)/18)*9;
    //console.log('mid:' + mid);
